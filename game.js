...
function revealCard(card) {
  ...
  if (correct && order === currentStep) {
    ...
    updateProgress();
    setTimeout(() => {
      if (currentStep > 4) finishGame(false); // ✅ completou
      else {
        renderCards();
        isClickable = true;
      }
...
function startTimer() {
  ...
  timer = setInterval(() => {
    ...
    timeLeft = Math.max(90 - elapsed, 0);
    timerEl.innerText = `Tempo restante: ${timeLeft}s`;

    if (timeLeft <= 0) {
      clearInterval(timer);
      finishGame(true); // ⛔ tempo acabou
    }
  }, 1000);
}
...
function finishGame(timeOut) {
  ...
  localStorage.setItem("gameCompleted", (!timeOut && currentStep > 4) ? "true" : "false");
